...
const colorStyles: Record<
  FeatureColor,
  { icon: string; bg: string; border: string; gradient: string }
> = {
  blue: {
    icon: "text-blue-400",
    bg: "bg-blue-500/10",
...
export function FeatureCard({ icon, title, color = "blue" }: FeatureCardProps) {
  ...
  return (
    <div
      className={`relative flex items-center gap-4 p-4 rounded-2xl bg-[var(--cor-escuro-6)] border border-[var(--cor-escuro-7)] hover:${styles.border} transition-all overflow-hidden group`}
    >
      <div className={`absolute inset-0 ${styles.gradient}`} />

      <div
        className={`relative flex items-center justify-center w-12 h-12 rounded-xl ${styles.bg} ${styles.border} border ${styles.icon}`}
      >
        {icon}
      </div>
